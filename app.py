...
st.set_page_config(page_title="Stakeholder Friction Mapping", page_icon="🧩", layout="wide")

st.title("🧩 Stakeholder Friction Mapping - AI Simulation")
st.markdown("This application runs a **CrewAI** simulation where AI agents play a workshop game to map friction points in Danish municipal aid assessment processes.")
...
3. [Suggestion 3]''',
    ...
    game = Crew(
        ...
        verbose=False # Silenced in console so UI flows well
    )

    return game.kickoff()

# --- UI ELEMENTS ---
with st.expander("ℹ️ Simulation Details", expanded=True):
    st.markdown("""
    - **Objective**: Evaluate the usability and game mechanics (Task Trackers, Lightning Bolts, Method Cards) designed for municipalities.
    - **Scenario**: Visually impaired citizen applies for a screen reader after an ophthalmologist referral.
    - **AI Agents Playing**: 
      1. Game Facilitator
      2. Municipal Caseworker (Hilleroed)
      3. Communication Center Specialist (Rudersdal)
      4. Visually Impaired Citizen
      5. Doctor (Ophthalmologist)
    - **Model**: `Llama 3.1 8B` (Via local Ollama)
    """)
...
if 'running' in st.session_state and st.session_state['running']:
    with st.spinner("The simulation is running. AI agents are interacting with each other... (This will take a few minutes, do not close this tab!)"):
...
if 'result' in st.session_state:
    ...
    st.header("📊 Final Workshop Report")
    
    # Extract raw text from CrewAI result
    final_text = st.session_state['result'].raw if hasattr(st.session_state['result'], 'raw') else str(st.session_state['result'])
    ...
    st.download_button(
        ...
        data=final_text,
        file_name="workshop_simulation_report.md",
        mime="text/markdown"
    )
